...
import { CheckCircle2, ChevronLeft, ChevronRight, Circle, Clock, Inbox, LogIn, LogOut, MapPin, Users } from 'lucide-react';
 ...
 * - Principal: picks a class teacher (reqEmail) from a dropdown and views their class roster, read-only.
...
export default function AttendancePageContent({ role }: { role: Role }) {
  ...
  useEffect(() => {
    ...
    getAllTeacherStaff()
      // Only teachers who actually have a class assigned show up here — the
      // roster endpoint needs reqEmail to resolve to a real class teacher.
      .then((res) => setTeachers(res.content.filter((t) => t.assignedClasses.length > 0)))
      .catch(() => setTeachers([]))
  ...
  useEffect(() => {
    ...
    setRosterError('');
    getStudentsByClassTeacher({ reqEmail, page, size: PAGE_SIZE })
      .then((res) => {
        if (cancelled) return;
      ...
      {!isTeacher && (
        <div className="card-premium p-4">
          <SelectField
            ...
            hint="Pick a teacher to view the roster of the class they teach."
          >
            <option value="">{teachersLoading ? 'Loading teachers…' : 'Select a class teacher'}</option>
