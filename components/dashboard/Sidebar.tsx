...
export default function Sidebar({ role, open, collapsed, onToggleCollapsed, onClose, onLogout }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-72 shrink-0 flex-col overflow-hidden bg-gradient-to-b from-amber-950 via-[#2b1706] to-stone-950 text-slate-100 shadow-premium-lg transition-all duration-300 ease-out lg:static lg:translate-x-0 ${
      ...
      >
        ...
        <button
          ...
          className="absolute top-6 -right-3 z-10 hidden h-6 w-6 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 shadow-premium-sm transition-all hover:text-amber-700 hover:shadow-premium lg:flex"
        >
        ...
        <div className="relative flex h-16 shrink-0 items-center justify-between px-4">
          <div className="flex min-w-0 items-center gap-2.5 font-semibold">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-amber-400 to-amber-600 text-amber-950 shadow-premium-sm">
