        ...
        {THEME_PRESETS.map((preset, idx) => (
          <div
            ...
            className={`animate-fade-in-up group relative flex flex-col overflow-hidden rounded-xl border bg-white transition-all duration-300 hover:-translate-y-1 hover:shadow-premium-lg ${
              preset.recommended ? 'border-brand-300 ring-1 ring-brand-100' : 'border-slate-200'
            }`}
          >
            {preset.recommended && (
              <span className="absolute top-2 right-2 z-10 rounded-full bg-brand-600 px-2 py-0.5 text-[10px] font-semibold text-white shadow-premium-sm">
                Recommended
              </span>
            )}
            ...
            <div className="flex flex-1 flex-col p-3.5">
              ...
              <button
                ...
                className="mt-3 flex items-center justify-center gap-1.5 rounded-md border border-brand-200 px-3 py-1.5 text-xs font-semibold text-brand-700 transition-colors hover:bg-brand-600 hover:text-white"
              >
