...
export default function SegmentedTabs({ tabs, active, onChange, fullWidth = false }: SegmentedTabsProps) {
  return (
    <div className={`inline-flex w-full rounded-2xl bg-brand-50/70 p-1.5 ring-1 ring-inset ring-brand-100 ${fullWidth ? '' : 'sm:w-auto'}`}>
      {tabs.map((tab) => {
        const Icon = tab.icon;
        ...
        return (
          <button
            ...
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-xl px-4 py-2 text-sm font-semibold transition-all ${fullWidth ? '' : 'sm:flex-none'} ${
              isActive ? 'bg-white text-brand-700 shadow-premium-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
            <Icon size={15} />
            {tab.label}
